...
const Button = forwardRef<HTMLButtonElement, ButtonProps>(
  ({ variant = 'primary', className = '', children, ...props }, ref) => {
    ...
    /* ── Outer shell — static amber border, soft ambient glow ── */
    const outerStyle: React.CSSProperties = {
      ...
      borderColor: isPrimary
        ? 'rgba(212,168,75,0.50)'
        : 'rgba(212,168,75,0.28)',
      boxShadow: isPrimary
        ? [
            '0 4px 16px rgba(0,0,0,0.50)',
            '0 0 18px rgba(212,168,75,0.14)',
            'inset 0 1px 0 rgba(255,255,255,0.06)',
          ].join(', ')
        : [
            '0 4px 14px rgba(0,0,0,0.40)',
            '0 0 10px rgba(212,168,75,0.07)',
          ].join(', '),
    };
    ...
    const catchLightStyle: React.CSSProperties = {
      ...
      right: 0,
      boxShadow: isPrimary
        ? '0 0 18px rgba(255,248,220,0.22)'
        : '0 0 12px rgba(212,168,75,0.16)',
      pointerEvents: 'none',
      zIndex: 0,
    };
    ...
    const blobStyle: React.CSSProperties = {
      ...
      boxShadow: isPrimary
        ? '-8px 8px 28px rgba(212,168,75,0.22)'
        : '-6px 6px 20px rgba(212,168,75,0.10)',
    };
    ...
    const innerTintStyle: React.CSSProperties = {
      ...
      pointerEvents: 'none',
      background: isPrimary
        ? 'radial-gradient(circle 60px at 0% 100%, rgba(232,192,96,0.18), rgba(184,136,26,0.08), transparent)'
        : 'radial-gradient(circle 50px at 0% 100%, rgba(212,168,75,0.12), rgba(184,136,26,0.06), transparent)',
      zIndex: 0,
    };

    /* ── Hover: glow brightens, border sharpens ── */
    const hoverAnim = isPrimary
      ? {
          borderColor: 'rgba(212,168,75,0.75)',
          boxShadow: [
            '0 6px 22px rgba(0,0,0,0.55)',
            '0 0 28px rgba(212,168,75,0.30)',
            '0 0 8px rgba(212,168,75,0.18)',
            'inset 0 1px 0 rgba(255,255,255,0.08)',
          ].join(', '),
          filter: 'brightness(1.08)',
        }
      : {
          borderColor: 'rgba(212,168,75,0.48)',
          boxShadow: [
            '0 4px 16px rgba(0,0,0,0.45)',
            '0 0 20px rgba(212,168,75,0.18)',
          ].join(', '),
